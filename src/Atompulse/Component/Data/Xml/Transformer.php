...
/**
 * Based on the source code from: https://github.com/mrusca/xml2array
 *
 * Transformer: A class to convert XML to an array in PHP
 ...
 *
 * Usage:
 *       $array = Transformer::createArray($xml);
 *       $array = Transformer::createArray($xml, array('useNamespaces' => true));
 */
class Transformer
{
    /**
    ...
    {
        $instance = new Transformer($config);
    ...
    public function &buildArray($inputXml)
    {
        ...
        if (is_string($inputXml)) {
            ...
            if ( ! $parsed) {
                throw new \Exception('[Transformer] Error parsing the XML string.');
            }
        } elseif ($inputXml instanceof \DOMDocument) {
            $this->xml = $inputXml;
        } else {
            throw new \Exception('[Transformer] The input XML object should be of type: DOMDocument but got instead ['.gettype($inputXml).'].');
        }
