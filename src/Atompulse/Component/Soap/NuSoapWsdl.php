 ...
 * @version  $Id: class.nuSoapWsdl.phpdl.php,v 1.76 2010/04/26 20:15:08 snichol Exp $
 * @access public
 */
class NuSoapWsdl extends NuSoapBase
{
    ...
    var $endpoint = '';
    // array of NuSoapWsdl docs to import
    var $import = array();
    // parser vars
    var $parser;
    var $position = 0;
    ...
    /**
     * parses the NuSoapWsdl document
     *
    ...
    function parseWSDL($wsdl = '')
    {
        ...
        // parse $NuSoapWsdl for url format
        $wsdl_props = parse_url($wsdl);

        if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme'] == 'https')) {
            $this->debug('getting WSDL http(s) URL ' . $wsdl);
            // get NuSoapWsdl
            $tr = new NuSoapTransportHttp($wsdl, $this->curl_options, $this->use_curl);
            ...
            // $NuSoapWsdl is not http(s), so treat it as a file URL or plain file path
    ...
    /**
     * serialize the parsed NuSoapWsdl
     *
    ...
    function serialize($debug = 0)
    {
        ...
        // 10.9.02 - add poulter fix for NuSoapWsdl and tns declarations
    ...
    function serializeType($name, $type, $value, $use = 'encoded', $encodingStyle = false, $unqualified = false)
    {
        ...
        // if a NuSoapVal has been supplied, let its type override the WSDL
        if (is_object($value) && get_class($value) == 'NuSoapVal') {
            if ($value->type_ns) {
                $type = $value->type_ns . ':' . $value->type;
                $forceType = true;
                $this->debug("in serializeType: NuSoapVal overrides type to $type");
            } elseif ($value->type) {
                $type = $value->type;
                $forceType = true;
                $this->debug("in serializeType: NuSoapVal overrides type to $type");
                ...
                $forceType = false;
                $this->debug("in serializeType: NuSoapVal does not override type");
            }
            $attrs = $value->attributes;
            $value = $value->value;
            $this->debug("in serializeType: NuSoapVal overrides value to $value");
            if ($attrs) {
                if (!is_array($value)) {
                    $value['!'] = $value;
                }
                ...
                $this->debug("in serializeType: NuSoapVal provides attributes");
            }
        } else {
            $forceType = false;
        }
     ...
     * @param array $attrs e.g. array(array('ref'=>'SOAP-ENC:arrayType','NuSoapWsdl:arrayType'=>'xsd:string[]'))
